...
CREATE USER second IDENTIFIED BY second;

GRANT CREATE SESSION TO second;
GRANT CREATE TABLESPACE TO second;
GRANT CREATE TABLE TO second;
GRANT CREATE SEQUENCE TO second;
alter user second default tablespace users quota unlimited on users;
```
...
CREATE TABLE product(
    ...
    pcontent clob,
    preadCount number default 0,
    pcreateDate timestamp,
    foreign key (puserId) references member (id)
);
